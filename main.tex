...
\title{\textbf{An Introduction to \\ Simplicial Homology Theory} \\[0.2cm] \large From Simplicial to Singular Homology}
...
\cdots \rightarrow A_{d+1} \xrightarrow{\alpha_{n+1}} A_{d} \xrightarrow{\alpha_d} \rightarrow A_{d-1} \xrightarrow{} \cdots
\end{equation}
...
\begin{enumerate}
	...
	\item[2.] \glqq $\Rightarrow$\grqq{}: First, assume that $A \xrightarrow{a} B \rightarrow 0$ is exact. This means that $\text{im}(a) = \ker(0)$, which implies that $\text{im}(a) = B$ since the kernel of the zero map is always the entire group. Therefore, $\text{im}(a) = B$.
